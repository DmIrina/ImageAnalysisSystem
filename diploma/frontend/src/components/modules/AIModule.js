...
export default function AIModule({ results, originalFileName }) {
    ...
    return (
        <div>
            <h3 className="text-lg font-semibold mb-2">AI-детекція</h3>

            <p className="mb-3">
                Ймовірність того, що зображення є згенерованим ШІ:{" "}
                <b>{probText}</b>
            </p>

            <p className="text-sm text-gray-600 mb-2">
                Модуль аналізує глобальні ознаки зображення (ViT) і
                повертає карту важливих ділянок, які найбільше вплинули
...
// import HeatmapCanvas from "../HeatmapCanvas";
//
// export default function AIModule({ results }) {
//     const aiScore = results?.ai_score ?? null;
//     const aiHeatmap = results?.ai_heatmap ?? null;
//
//     const probText =
